...
class FlappyBird:
    ...
    def cycle(self):
        ...
        # Смэрть
        if uRect.colliderect(self.bird):
            self.dead = True
        # Смэрть
        if dRect.colliderect(self.bird):
            self.dead = True
        ...
        if not 0 < self.bird[1] < 720: # если улетела - возраждаем
            ...
            self.Y_axis = 50
            self.dead = False
            if self.set_score < self.counter:
                self.set_score = self.counter
            if self.set_score > self.score:
    ...
    def run(self):
        ...
        while True:
            ...
            if not self.flag:
                ...
                self.screen.blit(font.render("press any button",
                                             -1,
                                             (255, 255, 255)),
                                 (70, 50))
